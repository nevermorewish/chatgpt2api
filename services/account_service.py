...
class AccountService:
    ...
    def __init__(self, storage_backend: StorageBackend):
        ...
        self._lock = Lock()
        self._index = 0
        self._busy_image_tokens: set[str] = set()
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        normalized["owner_user_id"] = self._clean_token(normalized.get("owner_user_id")) or None
    ...
    def _identity_owner_user_id(self, identity: dict[str, Any] | None) -> str | None:
        if not isinstance(identity, dict):
            return None
        if self._clean_token(identity.get("role")) != "user":
            return None
        return self._clean_token(identity.get("id")) or None

    def _account_matches_identity(self, account: dict, identity: dict[str, Any] | None) -> bool:
        owner_user_id = self._identity_owner_user_id(identity)
        if owner_user_id is None:
            return True
        return self._clean_token(account.get("owner_user_id")) == owner_user_id

    def _public_items(self, accounts: list[dict], *, include_sensitive: bool = True) -> list[dict]:
        return [
            {
                "id": hashlib.sha1(access_token.encode("utf-8")).hexdigest()[:16],
                "access_token": access_token if include_sensitive else "",
                ...
                "email": account.get("email") if include_sensitive else None,
                "user_id": account.get("user_id") if include_sensitive else None,
                "limits_progress": (account.get("limits_progress") or []) if include_sensitive else [],
                "default_model_slug": account.get("default_model_slug") if include_sensitive else None,
    ...
    def _list_available_candidate_tokens(
        ...
        *,
        ...
        busy_tokens = self._busy_image_tokens if idle_only else set()
        return [
            token
    ...
    def _pick_next_candidate_token(
        ...
        with self._lock:
            all_tokens = self._list_available_candidate_tokens(excluded_tokens, identity)
            if not all_tokens:
                raise RuntimeError("no available image quota")
            tokens = self._list_available_candidate_tokens(excluded_tokens, identity, idle_only=True)
            if not tokens:
                raise RuntimeError("free image pool is busy, please retry later")
            access_token = tokens[self._index % len(tokens)]
            self._index += 1
            self._busy_image_tokens.add(access_token)
            return access_token

    def release_access_token(self, access_token: str) -> None:
        normalized = self._clean_token(access_token)
        if not normalized:
            return
        with self._lock:
            self._busy_image_tokens.discard(normalized)
    ...
    def get_available_access_token(self, identity: dict[str, Any] | None = None) -> str:
        attempted_tokens: set[str] = set()
        while True:
            access_token = self._pick_next_candidate_token(excluded_tokens=attempted_tokens, identity=identity)
    ...
    def get_text_access_token(self, identity: dict[str, Any] | None = None) -> str:
        with self._lock:
    ...
    def has_available_account(self, identity: dict[str, Any] | None = None) -> bool:
        with self._lock:
    ...
    def list_accounts(self, identity: dict[str, Any] | None = None) -> list[dict]:
        with self._lock:
            accounts = [item for item in self._accounts if self._account_matches_identity(item, identity)]
            include_sensitive = self._identity_owner_user_id(identity) is None
            return self._public_items(accounts, include_sensitive=include_sensitive)
    ...
    def delete_accounts(self, tokens: list[str]) -> dict:
        ...
        with self._lock:
            ...
            self._busy_image_tokens.difference_update(target_set)
    ...
    def clear_owner(self, owner_user_id: str) -> int:
        normalized_owner = self._clean_token(owner_user_id)
        if not normalized_owner:
            return 0
        changed = 0
        with self._lock:
            for index, item in enumerate(self._accounts):
                if self._clean_token(item.get("owner_user_id")) != normalized_owner:
                    continue
                account = self._normalize_account({**item, "owner_user_id": None})
                if account is None:
                    continue
                self._accounts[index] = account
                changed += 1
            if changed:
                self._save_accounts()
                log_service.add(LOG_TYPE_ACCOUNT, "清理用户账号归属", {"owner_user_id": normalized_owner, "count": changed})
        return changed
