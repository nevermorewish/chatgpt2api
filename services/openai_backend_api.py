...
class OpenAIBackendAPI:
    ...
    def _is_free_image_account(self, requirements: ChatRequirements | None = None) -> bool:
        persona = str((requirements.persona if requirements else "") or "").strip().lower()
    ...
    def _image_model_slug(self, model: str, requirements: ChatRequirements | None = None) -> str:
        """把标准图片模型名映射到底层 model slug。"""
        model = str(model or "").strip()
        if not model:
            return "auto"
        if model == "gpt-image-2":
            upstream_model = config.image_generation_upstream_model
            if upstream_model:
                logger.info({
                    "event": "image_model_strategy_override",
                    "requested_model": model,
                    "upstream_model": upstream_model,
                })
                return upstream_model
            if config.image_generation_strategy == "gpt2api" and self._is_free_image_account(requirements):
                logger.info({
                    "event": "image_model_strategy_gpt2api_free_auto",
                    "persona": str((requirements.persona if requirements else "") or ""),
                    "requested_model": model,
                    "upstream_model": "auto",
                })
                return "auto"
    ...
    def _image_system_hints(self) -> list[str]:
        hints = ["picture_v2"]
        if config.image_generation_enable_reasoning:
            hints.append("reason")
        logger.info({"event": "image_system_hints", "hints": hints})
        return hints

    def _access_token_payload(self) -> Dict[str, Any]:
        parts = str(self.access_token or "").split(".")
        if len(parts) < 2:
            return {}
    ...
    def _codex_account_id(self) -> str:
        account = account_service.get_account(self.access_token) if self.access_token else {}
        account = account if isinstance(account, dict) else {}
        for key in ("chatgpt_account_id", "account_id"):
            ...
            if value:
                return value
        auth = self._access_token_payload().get("https://api.openai.com/auth")
        if isinstance(auth, dict):
            value = str(auth.get("chatgpt_account_id") or "").strip()
            if value:
                return value
        return ""

    def _codex_installation_id(self) -> str:
        account = account_service.get_account(self.access_token) if self.access_token else {}
        account = account if isinstance(account, dict) else {}
        return str(account.get("installation_id") or account.get("codex_installation_id") or "").strip()
    ...
    def _codex_image_size(self, size: str | None) -> str:
    ...
    def _codex_image_quality(self, quality: str | None) -> str:
    ...
    def _codex_reasoning_effort(self, quality: str | None) -> str:
        if str(quality or "").strip().lower() == "xhigh":
            return "xhigh"
        return config.image_generation_codex_reasoning_effort

    def _codex_image_data_url(self, image: str) -> str:
        value = str(image or "").strip()
        if value.startswith("data:"):
            return value
        return f"data:image/png;base64,{value}"

    def _collect_codex_image_items(self, value: Any, items: list[Dict[str, str]], seen: set[str]) -> None:
        if isinstance(value, dict):
            if str(value.get("type") or "") == "image_generation_call" and value.get("result"):
                b64_json = str(value.get("result") or "")
                digest = b64_json[:64]
                if digest not in seen:
                    seen.add(digest)
                    items.append({
                        "b64_json": b64_json,
                        "revised_prompt": str(value.get("revised_prompt") or ""),
                    })
            for item in value.values():
                self._collect_codex_image_items(item, items, seen)
        elif isinstance(value, list):
            for item in value:
                self._collect_codex_image_items(item, items, seen)

    def generate_codex_image_items(
            self,
            prompt: str,
        ...
        """Use the private Codex Responses image_generation tool chain."""
        if not self.access_token:
            raise RuntimeError("access_token is required for codex image endpoints")

        content: list[Dict[str, str]] = [{"type": "input_text", "text": prompt}]
        for image in images or []:
            content.append({"type": "input_image", "image_url": self._codex_image_data_url(image)})

        tool: Dict[str, Any] = {
            "type": "image_generation",
            "output_format": "png",
            "size": self._codex_image_size(size),
            "quality": self._codex_image_quality(quality),
        }

        reasoning_effort = self._codex_reasoning_effort(quality)
        body: Dict[str, Any] = {
            "model": config.image_generation_codex_model,
            "instructions": "You are an image generation assistant. Use the image_generation tool when the user asks for an image.",
        ...
        if installation_id:
            body["client_metadata"] = {"x-codex-installation-id": installation_id}
        ...
        logger.info({
            ...
            "has_account_id": bool(self._codex_account_id()),
        ...
        ensure_ok(response, "codex_responses")
        ...
        try:
            for payload in iter_sse_payloads(response):
                ...
                if event_type in {"response.failed", "response.incomplete"}:
                    raise RuntimeError(f"codex image generation failed: {event}")
                if event_type == "response.image_generation_call.partial_image" and event.get("partial_image_b64"):
                    b64_json = str(event.get("partial_image_b64") or "")
                    digest = b64_json[:64]
                    if digest not in partial_seen:
                        partial_seen.add(digest)
                        partial_items.append({
                            "b64_json": b64_json,
                            "revised_prompt": str(event.get("revised_prompt") or ""),
                        })
                self._collect_codex_image_items(event, items, seen)
        finally:
            response.close()
        ...
        if items:
            return items
        if partial_items:
            logger.info({"event": "codex_image_partial_fallback", "count": len(partial_items)})
            return partial_items
        plan_type = response.headers.get("x-codex-plan-type") or "unknown"
        active_limit = response.headers.get("x-codex-active-limit") or "unknown"
        raise RuntimeError(
            "codex image generation completed without image_generation_call result "
            f"(plan_type={plan_type}, active_limit={active_limit})"
        )

    def _prepare_image_conversation(self, prompt: str, requirements: ChatRequirements, model: str) -> str:
        """为图片生成准备 conduit token。"""
        path = "/backend-api/f/conversation/prepare"
        system_hints = self._image_system_hints()
        payload = {
            "action": "next",
            "fork_from_shared_post": False,
            "parent_message_id": new_uuid(),
            "model": self._image_model_slug(model, requirements),
